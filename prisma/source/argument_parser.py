...
def parse_arguments():
    ...
    group1.add_argument('--pingAsObs', type=int, help="sets if ping value is used as observation", default=1) #控制用队列长度还是时延
    ...
    # group4.add_argument('--overlay_adjacency_matrix_path', type=str, help='Path to the adjacency matrix', default="examples/abilene/topology_files/overlay_adjacency_matrix.txt") #8.19考虑到传递给conweave的run.py这里又是相对路径,因此要修改为正确的相对路径
    group4.add_argument('--overlay_adjacency_matrix_path', type=str, help='Path to the adjacency matrix', default="../prisma/examples/abilene/topology_files/overlay_adjacency_matrix.txt")
    ...
    ##新增解析conweave启动参数
    group_conweave = parser.add_argument_group('Conweave simulation arguments')
    #group_conweave.add_argument('--conweave_path',type=str,help='Path to the conweave simulator folder',default='../conweave-ns3-main/')
    group_conweave.add_argument('--cc', type=str, default='dcqcn', help='CC mode')
    ...
    group_conweave.add_argument('--conweave_topo_dir',type=str,default='../conweave-ns3-main/config',help='Direction that stores all conweave topology txt files')
    #新增存放overlay_adjacency_matrix序号和交换机编号的映射文件路径和文件名
    group_conweave.add_argument('--map_dir',type=str,default="examples/abilene/topology_files",help='存放overlay_adjacency_matrix序号和交换机编号映射')
...
# ------------------------------------------------------------------
    import subprocess
    conweave_topo_dir = params['conweave_topo_dir']
    conweave_topo_file = params['topo']+'.txt' if not params['topo'].endswith('.txt') else params['topo']
    map_dir = params['map_dir']
    map_file = params['map_file'] + '.txt' if not params['map_file'].endswith('.txt') else params['map_file']

    in_path = os.path.join(conweave_topo_dir,conweave_topo_file)
    ...
    #传递index_to_switch_id映射文件路径
    params["index_to_switch_id_map_path"] = os.path.abspath(map_path)
    # 传递conweave topo文件的绝对路径（仅内部使用，不新增CLI参数）
    params["conweave_topo_file_path"] = os.path.abspath(in_path)
    
    ## add general env params depricated
    # 训练提速：将 stepTime 调到与 reward 窗口同量级（0.5~1ms）
    params["stepTime"]=0.0005
    params["startSim"]=0
