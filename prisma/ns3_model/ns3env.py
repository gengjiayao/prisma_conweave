...
from ns3_model.start_sim import start_sim_script, build_ns3_project

import ns3_model.messages_pb2 as pb
...
class Ns3ZmqBridge(object):
    ...
    def __init__(self, port=0, startSim=True, simSeed=0, simArgs={}, debug=False):
        ...
        # print("ICIIIIIIAAAAAAAAAAAAAAAAAAAAAAAAAAA")
    ...
    def initialize_env(self, stepInterval):
        ...
        if self.debug:
            print(f"[ZMQ RECV] 从ns3处接收 SimInitMsg, size: {len(request)} bytes")
        simInitMsg = pb.SimInitMsg()
        simInitMsg.ParseFromString(request)
        if self.debug:
            print(f"[ZMQ RECV] SimInitMsg content:\n{simInitMsg}")
        ...
        if self.debug:
            print(f"[ZMQ SEND] 发送 SimInitAck 到 ns-3, size: {len(replyMsg)} bytes")
    ...
    def rx_env_state(self):
        ...
        try:
            request = self.socket.recv()
            if self.debug:
                print(f"[ZMQ RECV] 从ns3接收 EnvStateMsg , size: {len(request)} bytes")
        
            envStateMsg = pb.EnvStateMsg()
            envStateMsg.ParseFromString(request)
            if self.debug:
                print(f"[ZMQ RECV] EnvStateMsg content:\n{envStateMsg}")
    ...
    def send_close_command(self):
        ...
        replyMsg = reply.SerializeToString()
        if self.debug:
            print(f"[ZMQ SEND] Sent EnvActMsg (close) to ns-3, size: {len(replyMsg)} bytes")
        self.socket.send(replyMsg)
        self.newStateRx = False
        return True
    ...
    def send_actions(self, actions):
        try:
            ...
            replyMsg = reply.SerializeToString()
            if self.debug:
                print(f"[ZMQ SEND] 发送动作到ns3, size: {len(replyMsg)} bytes, actions: {actions}")
            self.socket.send(replyMsg)
            self.newStateRx = False
            return True
        except:
    ...
    def step(self, actions):
        ...
        if self.debug:
            print(f"[ZMQ STEP] Step completed. Received state update: {res}")
        return res
...
class Ns3Env(gym.Env):
    def __init__(self, stepTime=0, port=0, startSim=True, simSeed=0, simArgs={}, debug=False):
        ...
        if self.debug:
            print(f"[ZMQ INIT] Ns3Env initialized on port {self.port}, waiting for state...")
    ...
    def step(self, action):
        ...
        #print(f"[ZMQ STEP] Action sent: {action}, connected: {self.connected}")
        self.envDirty = True
        return self.get_state()

    def reset(self):
        if self.debug:
            print(f"[ZMQ RESET] Environment reset requested, port: {self.port}")
        self.envDirty = False
