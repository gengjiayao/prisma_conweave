...
int main (int argc, char *argv[])
{
  ...
  for (int i = 0; i < overlay_n_nodes; i++)
  {
    //8.20关注一下overlay_to_underlay_map的内容功能
    ...
    //为每一个overlay节点创建OpenGym通道ZMQ
    Ptr<OpenGymInterface> openGymInterface = CreateObject<OpenGymInterface> (openGymPort + i);
    //为第i个overlay节点开一个OpenGymInterface(openGymPort + i)内部是zmq的socket
    ...
    packetRoutingEnv->mapOverlayNodes(underlay_to_overlay_map); //underlay到overlay映射
