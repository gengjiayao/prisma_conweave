...
#ifndef PING_FORWARD_PACKET_MANAGER_H
#define PING_FORWARD_PACKET_MANAGER_H
...
class PingForwardPacketManager : public PacketManager
{
public:
  PingForwardPacketManager();
  PingForwardPacketManager(Ptr<Node> node, vector<int> neighbors);

  bool receivePacket(Ptr<Packet> packet, Ptr<NetDevice> receivingNetDev);
  void sendPingBackPacket(float delay,  uint32_t overlayTunnelIndex, uint32_t pingPacketIndex);
