...
BigSignalingAppHelper::BigSignalingAppHelper (std::string protocol, Address address)
{
  m_factory.SetTypeId ("ns3::BigSignalingGeneratorApplication");
  m_factory.Set ("Protocol", StringValue (protocol));
  m_factory.Set ("Remote", AddressValue (address));
}

void 
BigSignalingAppHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
BigSignalingAppHelper::Install (Ptr<Node> node) const
{
  return ApplicationContainer (InstallPriv (node));
}

ApplicationContainer
BigSignalingAppHelper::Install (std::string nodeName) const
{
...
ApplicationContainer
BigSignalingAppHelper::Install (NodeContainer c) const
{
...
Ptr<Application>
BigSignalingAppHelper::InstallPriv (Ptr<Node> node) const
{
...
void 
BigSignalingAppHelper::SetAverageStep (float syncStep, uint32_t packetSize)
{
