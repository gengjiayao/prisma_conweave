...
if __name__ == '__main__':
    ...
    for signaling in (0, 1):
        ...
        for xxx, signaling_type in enumerate(["NN", "target", "sp", "opt"]):
            # for idx_mat in ['0', '1','2', '3']:
                ...
                for stat_idx in range(len(stat_names)):
                    ...
                    syncs =  [512,1024, 2500, 5000, 10000, 15000]
                    # syncs.remove(4500)
                    if signaling_type in ("ideal", "NN"):
                        names = [f"dqn_buffer_{signaling_type}_{signaling}_fixed_rb_{xx}_sync{2500}ms_ratio_10" for xx in syncs] 
                        official_names = [f"DQN Buffer {signaling_type} rb {xx}s" for xx in np.array(syncs)]
                
                    elif signaling_type == "target":
                        names = [f"dqn_buffer_{signaling_type}_{signaling}_fixed_rb_{xx}_sync{10000}ms_ratio_10" for xx in syncs] 
                        official_names = [f"DQN Buffer {signaling_type} rb {xx}s" for xx in np.array(syncs)]
                    ...
                    for i in range(len(names)):
                        for charge_index, charge in enumerate(list_charges):
                            ...
                            temp_all = np.loadtxt(f"D:\\NetSim_\\NetSim\\prisma\\examples\\abilene\\tests_4\\{names[i]}_load_{charge}.txt", delimiter=',', dtype=object)
                            # print(temp)
                            for idx_mat in ['0', '1','2', '3']:
                            # if True:
        ...
        # ax2.set_yl1im(0, )
        ax1.set_ylabel("Average Cost Per Packet", fontweight="bold")
    ...
    # ax3.hlines(all_data["Shortest Path Routing_0_3_tests_3"],
    #            0 ,
    #            12,
    ...
    #            linestyle="dashed",
    #            linewidth=7)
    # ax3.hlines(all_data["LP - Min Cost MCF_0_3_tests_3"],
    #            0 ,
    #            12,
    ...
    # ax3.set_ylabel("Average Cost Per Packet", fontweight="bold")
