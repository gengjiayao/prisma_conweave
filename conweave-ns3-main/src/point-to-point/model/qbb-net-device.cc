...
void QbbNetDevice::Resume(unsigned qIndex) {
    ...
    //9.3修改
    if (!m_paused[qIndex]) {
        // 队列本就未暂停，RESUME 属于冗余/越序，直接忽略即可
        return;
    }
    m_paused[qIndex] = false;
    if (m_resumeEvt[qIndex].IsRunning()) {
        m_resumeEvt[qIndex].Cancel();
    }
    DequeueAndTransmit();
}
...
bool QbbNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber) {
    ...
    //9.3修改
    if (!m_qbbEnabled) {
        // 退回基类：如果当前没有启用Qbb行为，可直接用PointToPoint默认发送逻辑
        return PointToPointNetDevice::Send(packet, dest, protocolNumber);
    }

    // 给数据包加上二层头（类似你在 SendPfc 里做的）
    AddHeader(packet, protocolNumber);

    // 用 CustomHeader 解析一下，供 SwitchSend 分类使用
    CustomHeader ch(CustomHeader::L2_Header | CustomHeader::L3_Header | CustomHeader::L4_Header);
    packet->PeekHeader(ch);

    // 最简单的做法：默认走优先级队列0。若未来要细分优先级，可在此根据 ch 决策 qIndex。
    uint32_t qIndex = 0;
    return SwitchSend(qIndex, packet, ch);
}
...
uint32_t QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type) {
    ...
    // ------- 9.3新增：安全获取源IP，或退回0.0.0.0 -------
    Ipv4Address src = Ipv4Address(); // 0.0.0.0
    Ptr<Ipv4> ipv4 = m_node->GetObject<Ipv4>();
    if (ipv4)
    {
        // 用“设备→IPv4接口”的正规映射，别用 m_ifIndex
        int32_t iface = ipv4->GetInterfaceForDevice(GetObject<NetDevice>());
        if (iface >= 0 && (uint32_t)iface < ipv4->GetNInterfaces()
            && ipv4->GetNAddresses(iface) > 0)
        {
            src = ipv4->GetAddress(iface, 0).GetLocal();
        }
        // else: 保持 0.0.0.0，表示这是个 L2-only 口（比如交换机）
    }
    // ---------------------------------------------
