...
OpenGymInterface::NotifyCurrentState() 
{
  ...
  // collect current env state 收集当前环境状态
  ...
  // send env state msg to python 发送给Python
  zmq::message_t request(envStateMsg.ByteSize());;
  envStateMsg.SerializeToArray(request.data(), envStateMsg.ByteSize());
  //NS_LOG_UNCOND("[ZMQ SEND] 环境状态信息发送给 Python, size: " << envStateMsg.ByteSize());
  m_zmq_socket.send (request, zmq::send_flags::none);

  // receive act msg form python 接受动作回复，执行
  ns3opengym::EnvActMsg envActMsg;
  zmq::message_t reply;
  (void) m_zmq_socket.recv (reply, zmq::recv_flags::none);
  //NS_LOG_UNCOND("[ZMQ RECV] 从Python接收环境状态信息, size: " << reply.size());
