...
public:
    ...
    // 专为 ns3::Ptr<T>，用 Get() 打印裸指针地址
    template <typename T>
    ParameterLogger& operator<<(const ns3::Ptr<T>& ptr)
    {
        if (m_itemNumber > 0) m_os << ", ";
        m_os << static_cast<const void*>(ptr.Get());
        m_itemNumber++;
        return *this;
    }
    ...
    // 裸指针类型
    template<typename T>
    ParameterLogger& operator<<(T* ptr)
    {
        if (m_itemNumber > 0) m_os << ", ";
        m_os << static_cast<const void*>(ptr);
        m_itemNumber++;
        return *this;
    }

    // 泛型（包括 int、string、自定义类型等）
    template<typename T>
...
public:
    ...
    /* ---------- 1. ns3::Ptr<T> ---------- */
    template <typename T>
    ParameterLogger& operator<<(const ns3::Ptr<T>& p)
    {
        if (m_itemNumber++) m_os << ", ";
        m_os << static_cast<const void*>(p);   // Ptr<T> 可隐式转裸指针
        return *this;
    }

    /* ---------- 2. 裸指针（排除函数指针）---------- */
    template <typename T>
    typename std::enable_if<!std::is_function<T>::value, ParameterLogger&>::type
    operator<<(T* p)
    {
        if (m_itemNumber++) m_os << ", ";
        m_os << static_cast<const void*>(p);
        return *this;
    }

    /* ---------- 3-A. *非 const* 左值，需要保留 ---- */
    template <typename T>
    ParameterLogger& operator<<(T& param)          // 与 PacketFilter& 等匹配
    {
        if (m_itemNumber++) m_os << ", ";
        m_os << param;
        return *this;
    ...
    /* ---------- 3-B. 通用兜底：右值 + const 左值 ---- */
    template <typename T>
    typename std::enable_if<
        !std::is_pointer<T>::value &&
        !std::is_reference<T>::value,              // 捕获右值/值语义
        ParameterLogger&>::type
